...
const importLine = 'import { billingRouter } from "./modules/billing";';
if (!text.includes(importLine)) {
  text = `${importLine}\n${text}`;
}

const mountLine = 'app.use("/api/v1/billing", billingRouter);';
if (!text.includes(mountLine)) {
  const anchor = 'app.use("/api/v1/pharmacy", pharmacyRouter);';

  if (!text.includes(anchor)) {
    throw new Error("Pharmacy route mount was not found in src/app.ts");
  }
...
console.log("Billing application route applied successfully");
